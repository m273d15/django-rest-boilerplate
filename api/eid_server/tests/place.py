...
from eid_server.eid_types.place import ICAOCountry, Place, GeneralPlace
...
class PlaceTests(TestCase):
    ...
    def test_valid_place_with_all_optional(self):
        place = Place(
            self.VALID_CITY, self.VALID_COUNTRY, state = self.VALID_STATE, 
            street = self.VALID_STREET, zip_code = self.VALID_ZIP_CODE)
